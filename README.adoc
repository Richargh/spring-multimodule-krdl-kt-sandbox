...
Most classes that we create cannot do their job alone. They need collaborators, other classes. That means if we test a class, make it a system-under-test (SUT), we also test the collaborator. Should we do that or should we replace the collaborator with a link:http://xunitpatterns.com/Test%20Double.html[test double]? To answer that question with a healthy dose of "it depends" we need to first look at the several variations of a test double with their intends. It's time for some code.
...
<2> collaborator
<3> direct output of sut
<4> direct input of sut
<5> indirect input of sut
<6> indirect output of sut
