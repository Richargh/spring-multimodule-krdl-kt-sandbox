...
Also, due to their reflective nature, it takes mocking frameworks like mockito or mockk about 1000 times longer to instantiate, configure and execute a method on a simple object like `open class DeepStub { open fun answer()= 42; val map = ConcurrentHashMap<String, String>()}`. Granted we only go from 0.0004 ms for constructor invocation to 0.2 ms (mockito: `val deep = mock(DeepStub::class.java); `when`(deep.answer()).thenReturn(42); deep.answer()`) or 0.3 ms (mockk: `val deep = mockk<DeepStub>(); every { deep.answer() }.returns(42); deep.answer()`) and that is often insignificant.
